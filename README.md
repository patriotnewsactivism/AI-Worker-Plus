...
Before building for production, create a `.env` (or `.env.local`) file at the project root that defines the Firebase credentials provisioned for your deployment:
...
All six values are required in production environments. When running locally (`npm run dev`), any missing value will fall back to the demo Firebase project that ships with the repository, allowing you to explore the UI without custom credentials.
